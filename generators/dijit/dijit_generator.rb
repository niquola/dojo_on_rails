      ...
      x   - just placeholder do not change defaults
        ...
        def manifest
          record do |m|
            ...
            @css_path=File.join(@root_package_path,'themes',DojoConfig.dojo.theme,packages[1..(packages.length-1)].join('/'))
            @test_path=File.join(@root_package_path,'tests',packages[1..(packages.length-1)].join('/'))
            ...
            assigns={
              ...
              :css_path=>@css_path.split('public')[1]
            }
            ...
            m.template "dijit.css", File.join(@css_path, "#{@class_name}.css"), :assigns=>assigns if @generate_css
            m.template "test.html", File.join(@test_path,"#{@class_name}.html"),:assigns=>assigns if @generate_test_page
            m.template "test.js", File.join(@test_path,"#{@class_name}.js"),:assigns=>assigns if @generate_test_page
